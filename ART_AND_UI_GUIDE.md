...
## 2. HUD and interaction (Implemented)
...
Every visual and sound in the prototype is **procedural and original**. It's drawn with Canvas paths in `src/client/render.ts` or synthesized in `src/client/audio.ts`. There are no image, font, model or audio files, and no generated concept art was produced. The placeholders are deliberate and consistent, but they are placeholders.
...
| HUD layout, colours, CSS | Hand-written, original | Project-owned | `src/client/index.html` | Implemented | Visual design pass |
...
| Concept art | None produced | — | — | — | Generated concept art, if made, goes in `art/concept/`, marked non-shippable until cleared |

Third-party code shipped in the build: **none**. Dev-only tools: esbuild (MIT), TypeScript (Apache-2.0), tsx (MIT), Playwright (Apache-2.0), @types/node (MIT).
